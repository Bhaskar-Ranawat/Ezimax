...
const { Schema } = mongoose; // Destructuring mongoose object
let commonJs = require("../config/common");
...
// {
//   "id": "e5aaf220-b854-4b18-8170-e1634779d7cc",
//   "skuId": "Buger233",
//   "price": 500,
//   "discountedPrice": 99,
//   "discountPercent": 81,
...
}, { collection: "VendorProduct" }, { timestamps: true });
 
// Compile the schema into a model
// const Product = mongoose.model('VendorProduct', businessProductSchema);
 
module.exports = businessProductSchema;
...
businessProductSchema.pre("save", function (next) {
  ...
  if (this.variantDetails && this.variantDetails.variants) {
    this.variantDetails.variants.forEach(variant => {
      const variantId = variant._id ? variant._id.toString() : new mongoose.Types.ObjectId().toString(); // Ensure variant has an ID
  ...
  if (doc) {
    ...
    if (doc.variantDetails && doc.variantDetails.variants) {
      doc.variantDetails.variants.forEach(variant => {
        const variantId = variant._id ? variant._id.toString() : new mongoose.Types.ObjectId().toString();
...
// Knowwu - Smart Digital Business Card - Sustainable, NFC and Eco friendly Card
// Knowwu redefines business cards with eco-friendly, NFC-powered digital solutions. Share contact info instantly, track interactions, and go green - all with one stylish card.
